...
const Skills = () => {
  ...
  const addSkill = () => {
    setValue({
      ...value,
      skills: [...skills, { id: skills.length + 1, name: currentSkill, desc: currentDesc }],
    });
    setCurrentSkill("");
    setCurrentDesc("");
  };
  ...
  return (
    <>
      <div className="m-2 flex items-center">
        <label htmlFor="skillInput" className="pr-4 text-xl font-light w-1/6">
          Skills
        </label>
        ...
        <button
          className="border rounded border-pink-300 hover:bg-pink-200 p-2 m-2 h-full disabled:bg-gray-200 disabled:border-gray-300"
          disabled={currentSkill.length == 0}
          onClick={addSkill}
        >
          +
        </button>
      ...
      <div className="m-2 flex flex-row flex-wrap justify-center">
        {skills?.filter(Boolean).map((skill) => {
          return (
            <div key={skill.id} className="border rounded p-1 m-1 w-1/3">
              <button className="ml-auto w-full" onClick={() => deleteSkill(skill.id)}>
                X
              </button>
              <p className="p-2 text-md font-semibold hover:text-cyan-800">{skill.name} </p>
              <p className="p-2 text-md font-mono">{skill.desc}</p>
            </div>
          );
        })}
      </div>
  ...
  return (
    <div className="border border-black rounded w-1/2 p-6 m-2">
      <div className="flex flex-col">
        {FIELDS.map((field) => {
          return (
            <div key={field.camelizedLabel} className="m-2 flex items-center">
              <label htmlFor={field.camelizedLabel} className="pr-4 text-xl font-light w-1/6">
                {field.label}
              </label>
              <input
                onBlur={onBlur}
        ...
        <div className="m-2 flex items-center">
          <label htmlFor="profileInput" className="pr-4 text-xl font-light w-1/6">
            Profile
          </label>
          ...
          <textarea
            ...
            onBlur={onBlur}
            className="border border-black rounded p-2 w-2/3"
          ></textarea>
        </div>
